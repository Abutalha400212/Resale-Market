...
const Navbar = () => {
   ...
   const authItem = <>
  {user ? <button onClick={handleLogOut} className="btn btn-accent">Log Out</button> :<>  <li>
               <NavLink
      to='/login'
        className={({ isActive }) =>
          `flex items-center px-4 m-0.5 rounded-md transition-colors duration-300 transform  hover:bg-gray-300   hover:text-gray-700 ${
  ...
  return (
    ...
    <div className="navbar-start">
      <div className="dropdown">
        ...
        <ul tabIndex={2} className="menu menu-compact dropdown-content mt-3 p-2 shadow bg-transparent rounded-box w-52">
          {navItem}
        </ul>
      </div>
